...
type GonalSpace struct {
	Ctx           context.Context
	Cancel        context.CancelFunc
	Lock sync.Mutex
	Pool map[string]*Gonal
}
func (g *GonalSpace)Space(name string)*Gonal{
	g.Lock.Lock()
	defer g.Lock.Unlock()
	gonal, ok := g.Pool[name]
	if !ok {
		gonal = &Gonal{}
		g.Pool[name] = gonal
	}
...
var space = &GonalSpace{Ctx: Ctx, Cancel: Cancel, Pool: map[string]*Gonal{}}

func Notify(payload Payload)                  {}
func Bind(selector Label, handler ...Handler) {}
func Space(name string)                       {}
func Close(){}
